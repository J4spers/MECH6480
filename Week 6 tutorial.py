...
for steps in range(NUM_STEPS):
    ...
    #J_u_x = np.roll(J_u_x, 1)
    #print(J_u_y)
    #J_u_y = np.roll(J_u_y, 1, axis=1)
    #print(J_u_y)
    
    
    J_u_x = 0.25*(u[:-1,1:-1] + u[1:,1:-1])**2
    J_u_y[:-1,:] = 0.25*(u[1:-1,1:] + u[1:-1,:-1])*\
                 (v[2:-1,:] + v[1:-2,:])
                 
    J_u_y[-1,:] = 0.25*(u[-1,1:] + u[-1,:-1])*\
                 (v[-1,:] + v[-2,:])

    
    J_u_x -= nu*(u[1:,1:-1]-u[:-1,1:-1])/dx
    J_u_y[:-1,:] -= nu*(u[1:-1,1:]-u[1:-1,:-1])/dy
    
    J_u_y[-1,:] -= nu*(u[-1,1:]- u[-1,:-1])/dy
    #J_u_y = np.roll(J_u_y, 1, 0)
    ut[1:-1,1:-1] = u[1:-1,1:-1] - (DT/dxdy)*\
                    (J_u_x[1:,:]*dy - J_u_x[:-1,:]*dy + \
                     J_u_y[:-1,1:]*dx - J_u_y[:-1,:-1]*dx) + GX*DT
    #ut = np.roll(ut,2,1)            
    ...
    #vt[0,1:-1] = v[0,1:-1] - (DT/dxdy)*\
                #(J_u_y[-1,1:]*dx - J_u_y[-1,:-1]*dx)
    #vt = np.roll(vt,1,0)  #Maybe roll this im not sure      
    #J_v_y = np.roll(J_v_y, 1, 0)        
    #J_v_x = np.roll(J_v_x, 1, 0)  
